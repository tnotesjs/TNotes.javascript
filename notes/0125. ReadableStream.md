---
id: 4b2d81c1-3154-4cb6-a87a-39375ec845e0
description: ReadableStream 相关的核心概念及一些常见的基本操作
---
...
<html lang="zh-CN">
  ...
  <body>
    <h1>从数组创建 ReadableStream 并逐个读取元素</h1>

    <button onclick="demo1()">示例1：使用 reader.read() 读取</button>
    <button onclick="demo2()">示例2：使用 for await...of 读取</button>
    <button onclick="demo3()">示例3：尝试多个 reader（会报错）</button>
...
<html lang="zh-CN">
  ...
  <body>
    <h1>无限数据生成器流</h1>
    <p>演示如何创建按需生成数据的流，并支持取消操作</p>

    <button onclick="startCounter()">启动计数器</button>
    <button onclick="stopCounter()">停止计数器</button>
    <button onclick="startRandom()">启动随机数生成器</button>
    <button onclick="stopRandom()">停止随机数</button>
