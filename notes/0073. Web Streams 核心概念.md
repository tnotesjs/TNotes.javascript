---
id: 54545c00-8c6d-4086-b1cd-2f20d2440e1e
description: Web Streams API 的核心概念，包括可读流、可写流、转换流的基本原理和使用方法，以及流式处理相较于传统数据处理方式的优势。
---
...
```js [4]
// ----------------------------------------
// 🤔 为什么不直接拷贝从流中流出的数据，而是拷贝流？
// ----------------------------------------

// 原理上可行，但违背流式处理的初衷
// 使用流的目的很大程度上就是为了缓解内存的压力
// 流让我们能够分块读取数据，从而避免一次性将所有数据加载到内存中，导致“内存爆炸”

// ⚠️ 方案1. 拷贝数据
// 原理上可行，但违背流式处理的初衷
const response1 = await fetch('/large-video.mp4') // 1GB 视频
...
while (true) {
  const { done, value } = await reader.read()
  if (done) break
  chunks.push(value) // 问题：所有数据都在内存中！
}

// 现在需要拷贝整个数据
const data1 = new Uint8Array(chunks) // 1GB 内存
const data2 = new Uint8Array(chunks) // 又 1GB 内存

// 总共占用 2GB+ 内存！对于大文件，内存会爆掉

// ✅ 方案2. 分流
// 流式处理，内存友好
const response2 = await fetch('/large-video.mp4') // 1GB 视频
const [stream1, stream2] = response2.body.tee()

// stream1 和 stream2 会逐块读取数据
// 同一时刻内存中只有小块数据（比如 64KB）
// 处理完立即释放，内存占用极小
...
<html lang="zh-CN">
  <head>
    ...
    <title>对比传统 fetch().json() 与流式处理</title>
    <style>
      body {
        max-width: 900px;
        margin: 20px auto;
  ...
  <body>
    <h1>🎯 对比传统 fetch().json() 与流式处理</h1>
    <p>演示两种方式处理 JSON 数据的性能和用户体验差异</p>

    <div class="container">
      <!-- 传统方式 -->
      <div class="panel">
        <h3>❌ 传统方式：fetch().json()</h3>
        <button onclick="traditionalFetch()">开始请求</button>
        <div class="stats" id="traditional-stats">等待开始...</div>
        <div class="output" id="traditional-output"></div>
      </div>

      <!-- 流式处理 -->
      <div class="panel">
        <h3>✅ 流式处理：ReadableStream</h3>
        <button onclick="streamFetch()">开始请求</button>
        <div class="stats" id="stream-stats">等待开始...</div>
        <div class="output" id="stream-output"></div>
      </div>
    </div>
    ...
    <!-- 传统方式 -->
    <script src="./traditionalFetch.js"></script>

    <!-- 流式处理 -->
    <script src="./streamFetch.js"></script>
  </body>
</html>
```
