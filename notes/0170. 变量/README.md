...
- [1. 本节内容](#1-本节内容)
- [2. 评价](#2-评价)
- [3. 围绕“变量”展开的核心知识点汇总](#3-围绕变量展开的核心知识点汇总)
  - [3.1. 核心概念](#31-核心概念)
  - [3.2. 垃圾自动回收机制](#32-垃圾自动回收机制)
  - [3.3. 语法糖](#33-语法糖)
  - [3.4. 定义变量](#34-定义变量)
    - [`var`【已过时，不推荐使用】](#var已过时不推荐使用)
    - [`let`](#let)
    - [`const`](#const)
  - [3.5. 访问不存在的变量](#35-访问不存在的变量)
  - [3.6. 声明提升](#36-声明提升)
- [4. 内存、变量、值，三者之间的关系](#4-内存变量值三者之间的关系)
- [5. JavaScript 变量是“弱类型”的](#5-javascript-变量是弱类型的)
- [6. `var` 关键字](#6-var-关键字)
  - [6.1. `var` 关键字的作用域特性](#61-var-关键字的作用域特性)
  - [6.2. 如何看待 `var`？](#62-如何看待-var)
  - [6.3. 小结](#63-小结)
- [7. `let` 关键字](#7-let-关键字)
  - [7.1. 块级作用域](#71-块级作用域)
- [8. 用 `var` 声明的循环变量在循环中会出现什么问题？为什么使用 `let` 可以就可以解决此问题？【经典历史闭包陷阱问题】](#8-用-var-声明的循环变量在循环中会出现什么问题为什么使用-let-可以就可以解决此问题经典历史闭包陷阱问题)
- [9. `const` 关键字](#9-const-关键字)
  - [9.1. const 的 "常量约束" 本质](#91-const-的-常量约束-本质)
    - [原始类型](#原始类型)
    - [引用类型](#引用类型)
    - [栈内存、堆内存](#栈内存堆内存)
...
- [14. 面试题：下面的代码输出的结果是什么？](#14-面试题下面的代码输出的结果是什么)
- [15. 面试题：下面的代码输出的结果是什么？](#15-面试题下面的代码输出的结果是什么)
- [16. 面试题：请谈谈什么是变量声明提升？](#16-面试题请谈谈什么是变量声明提升)
- [17. 示例：`var` - 理解变量和值](#17-示例var---理解变量和值)
- [18. 示例：`var` - 区分大小写](#18-示例var---区分大小写)
- [19. 示例：`var` - 变量的声明、赋值](#19-示例var---变量的声明赋值)
- [20. 示例：`var` - 仅声明未赋值为 undefined](#20-示例var---仅声明未赋值为-undefined)
- [21. 示例：`var` - 隐式创建全局变量](#21-示例var---隐式创建全局变量)
- [22. 示例：`var` - 使用未声明的变量会报错](#22-示例var---使用未声明的变量会报错)
- [23. 示例：`var` - 可一次声明多个变量](#23-示例var---可一次声明多个变量)
- [24. 示例：`var` - 理解“动态”类型](#24-示例var---理解动态类型)
- [25. 示例：`var` - 变量重复声明无效](#25-示例var---变量重复声明无效)
- [26. 示例：`var` - 变量重复声明并重新赋值](#26-示例var---变量重复声明并重新赋值)
- [27. 示例：`var` - 无块级作用域](#27-示例var---无块级作用域)
- [28. 示例：`let` - 有块级作用域](#28-示例let---有块级作用域)
- [29. 示例：`let` - 使用 `var`、`let` 定义 `for` 循环的循环变量（经典的闭包陷阱问题）](#29-示例let---使用-varlet-定义-for-循环的循环变量经典的闭包陷阱问题)
- [30. 示例：`let` - `let` 暂时性死区](#30-示例let---let-暂时性死区)
- [31. 示例：`let` - 函数参数默认值中的死区](#31-示例let---函数参数默认值中的死区)
- [32. 示例：`let` - 其他奇怪的报错](#32-示例let---其他奇怪的报错)
- [33. 示例：`let` - 同一作用域内不允许重复声明](#33-示例let---同一作用域内不允许重复声明)
- [34. 示例：`let` - `for` 循环的参数和循环体处于不同的作用域](#34-示例let---for-循环的参数和循环体处于不同的作用域)
- [35. 示例：`let` - `let` 出现之前的经典历史问题](#35-示例let---let-出现之前的经典历史问题)
- [36. 示例：`const` - 常量不允许重新赋值](#36-示例const---常量不允许重新赋值)
- [37. 示例：`const` - 声明的同时完成初始化赋值](#37-示例const---声明的同时完成初始化赋值)
- [38. 示例：`const` - 引用类型，确保地址不变](#38-示例const---引用类型确保地址不变)
- [39. 示例：`const` - 对象冻结](#39-示例const---对象冻结)
- [40. 示例：`Hoisting` - `var` 声明](#40-示例hoisting---var-声明)
- [41. 示例：`TDZ` - `let` 和 `const` 声明](#41-示例tdz---let-和-const-声明)
- [42. 示例：`Hoisting` - 函数声明](#42-示例hoisting---函数声明)
- [43. 示例：`Hoisting` - 函数表达式](#43-示例hoisting---函数表达式)
- [44. 深入理解“提升”【不重要】](#44-深入理解提升不重要)
  ...
  - [44.6. 提升（Hoisting） ≠ 可访问（Accessible）、提升（Hoisting） ≠ 内存分配（Memory Allocation）](#446-提升hoisting--可访问accessible提升hoisting--内存分配memory-allocation)
...
这篇笔记围绕着“变量”这一主题展开，记录了不少内容，比如：`var`、`let`、`const` 这三个关键字的特性和区别，变量声明提升、暂时性死区等相关概念，以及一些经典的历史问题。像是内存、变量、值，这些概念是基础的通识，在学习其他编程语言时也一样会接触到。
...
定义变量：定义变量实际上是告诉计算机在内存中为该变量分配一定的空间。

给变量赋值：赋值操作是将某个数据值存储在变量关联的内存空间中。之后，每当我们引用这个变量，计算机就会到内存空间中读取其中的值。
...
变量可以在声明的时候同时完成赋值操作（语法糖），本质上是两步操作 => 声明 + 赋值。
...
## 5. JavaScript 变量是“弱类型”的

JavaScript 变量是“弱类型”的（也称“松散类型”、“动态类型”）。变量本身不固定绑定某一种数据类型，它只是某个值的名字。
...
相对而言，TypeScript 是“强类型”的，它是 JavaScript 的超集，在 JavaScript 基础上加上了类型系统。比如：

```ts
let message: string = 'hi'
message = 100 // ❌ TypeScript 会提示错误：类型“number”不能赋给类型“string”
```

关于 TypeScript 的更多信息，见知识库：[TNotes.typescript][8]。
...
虽然说 `var` 关键字已经退出历史舞台了，但是在一些开源项目中，还是会看到 `var` 的身影，所以还是有必要了解 `var` 关键字的一些基本特性，目的是为了能读懂别人用到 `var` 的程序。
...
`let` 具有块级作用域，声明的变量有暂时性死区，虽然变量声明提升了，但无法在声明语句之前访问变量，更加符合直觉。这里提到的“块级作用域”和“暂时性死区”也是 `const` 关键字具有的的特性。
...
const 实际上保证的，并不是变量的值不得改动，而是变量绑定的内存地址不得改动。

- 对于“原始类型”的数据（数值、字符串、布尔值），值就保存在变量绑定的内存地址中，因此等同于常量。
- 对于“引用类型”的数据（主要是对象和数组），变量绑定的内存地址保存的是一个指向实际数据的指针，const 只能保证这个指针是固定的（即总是指向同一个固定的地址），至于它指向的数据结构是不是可变的，就完全不能控制了。
...
在 JavaScript 中，变量声明提升（Hoisting）是一种行为，指的是变量和函数声明在代码执行之前被移动到它们各自作用域的顶部。这意味着对于 `var` 声明的变量和 `function` 声明的函数，可以在声明之前访问它们。但需要注意的是，`let` 和 `const` 声明的变量虽然在底层也会被“提升”（在词法环境创建时被注册），但在声明语句执行之前，它们处于暂时性死区（TDZ），无法在声明前访问，否则会抛出 `ReferenceError`。
...
因为函数声明 `function add(x) { return x + 40 }` 会被提升到作用域顶部，而 `var add` 的赋值不会提升，所以在执行 `add(x)` 时，调用的是函数声明版本的 `add`，而不是函数表达式版本的 `add`。
...
- `let` 和 `const` 也会被提升，但在执行到声明语句之前处于暂时性死区（TDZ），在此之前访问会抛出 `ReferenceError`。
...
对于使用未声明的变量，只有一种情况不会报错：
typeof 操作符检测变量的数据类型时，可以是未声明的变量，得到的结果是 "undefined"。
*/
```
...
## 29. 示例：`let` - 使用 `var`、`let` 定义 `for` 循环的循环变量（经典的闭包陷阱问题）
...
注意，暂时性死区也是 `const` 具备的特性，这里只是选择以 `let` 为例来介绍。
...
// 上面这行就属于这个情况，在变量 x2 的声明语句还没有执行完就去取 x2 的值，导致报错“ReferenceError”。
```

## 33. 示例：`let` - 同一作用域内不允许重复声明
...
## 34. 示例：`let` - `for` 循环的参数和循环体处于不同的作用域
...
![1](https://cdn.jsdelivr.net/gh/tnotesjs/imgs@main/2024-12-27-14-15-51.png)

![2](https://cdn.jsdelivr.net/gh/tnotesjs/imgs@main/2024-12-27-14-15-58.png)

:::

1. 无论点击哪个 Item
2. 最终都会打印 Item 6 clicked
...
1. 使用 `Object.freeze` 方法冻结对象，可以让对象变得不可变，无法添加、删除或修改属性。
2. 使用递归写法深度冻结对象。（简单版本）
...
在没有明确 Hoisting 是什么之前，针对这个问题结果的是“是”还是“否”的讨论是无意义的，因此需要先给提升下一个定义，然后自证观点。
...
### 44.6. 提升（Hoisting） ≠ 可访问（Accessible）、提升（Hoisting） ≠ 内存分配（Memory Allocation）

先理清几个常见的有关"提升的误解"：

- 提升（Hoisting） ≠ 可访问（Accessible）
- 提升（Hoisting） ≠ 内存分配（Memory Allocation）
...
变量声明在 JavaScript 引擎中经历三个阶段：

1. 创建（Create）：在词法环境实例化时，注册标识符并分配内存
2. 初始化（Initialize）：为已创建的变量设置初始值
3. 赋值（Assign）：将代码中指定的具体值绑定到变量（执行阶段）

`var`、`let`、`const` 提升内容对照表：

| 关键字 | 被提升的内容 | 表现示例 |
| --- | --- | --- |
| `var` | 创建 + 初始化（值为 `undefined`） | `console.log(a); var a=10;` -> `undefined` |
| `let` | 仅创建（未初始化） | `console.log(b); let b=20;` -> `ReferenceError` |
| `const` | 仅创建（未初始化） | `console.log(c); const c=30;` -> `ReferenceError` |
...
- 如果一门技术诞生的时间比较早，它很可能会存在一些历史遗留问题，如果要追这些问题的一个个“坑”点，成本会比较高。应该优先把重心放在那些更加重要且常用的知识点上，后续有机会再去补充这些“坑”点。
...
[8]: https://github.com/tnotesjs/TNotes.typescript
