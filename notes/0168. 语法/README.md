...
- [5. 转义符是什么？有什么用？](#5-转义符是什么有什么用)
  ...
  - [5.5. 转义符的英文为啥叫 `escape character`？【AI】](#55-转义符的英文为啥叫-escape-characterai)
  - [5.6. 转义符 `\r\n` 诞生的背景 - 旧版打字机【不重要】](#56-转义符-rn-诞生的背景---旧版打字机不重要)
- [6. 标识符是什么？标识符的命名规则是？](#6-标识符是什么标识符的命名规则是)
  - [6.1. 标识符是什么](#61-标识符是什么)
  - [6.2. 标识符的命名规则【重点】](#62-标识符的命名规则重点)
  - [6.3. 语义化](#63-语义化)
  - [6.4. 练习：判断以下哪些标识符是合法的？](#64-练习判断以下哪些标识符是合法的)
- [7. 注释和严格模式怎么用？](#7-注释和严格模式怎么用)
  - [7.1. 注释](#71-注释)
  - [7.2. 严格模式](#72-严格模式)
- [8. 问：我很少能在开源项目中看到显式写 `'use strict'` 的了，现代开发还用显式开启严格模式吗？【AI】](#8-问我很少能在开源项目中看到显式写-use-strict-的了现代开发还用显式开启严格模式吗ai)
- [9. 在使用分号和代码块 `{}` 时需要注意什么？](#9-在使用分号和代码块--时需要注意什么)
  - [9.1. 分号](#91-分号)
  - [9.2. 代码块](#92-代码块)
  - [9.3. 评价](#93-评价)
- [10. 问：什么是 ASI 机制？ASI 全称是什么？【AI】](#10-问什么是-asi-机制asi-全称是什么ai)
- [11. 引用](#11-引用)
...
### 5.5. 转义符的英文为啥叫 `escape character`？【AI】
...
## 8. 问：我很少能在开源项目中看到显式写 `'use strict'` 的了，现代开发还用显式开启严格模式吗？【AI】
...
## 10. 问：什么是 ASI 机制？ASI 全称是什么？【AI】

ASI 全称是 **Automatic Semicolon Insertion**，中文通常叫 **自动分号插入机制**。

它是 ECMAScript 的一种语法解析规则：当代码中省略了分号时，JavaScript 引擎会在解析阶段根据规则，尝试在合适的位置自动补上分号。
...
注意：ASI 不是简单地“每次换行都加分号”。如果下一行的内容可以和上一行组成合法语句，引擎可能不会插入分号。
...
它可能不会被理解成两条独立语句，而是尝试和上一行连在一起解析。

所以总结：

> ASI 是 JavaScript 在解析代码时，自动补充分号的机制。
>
> 它让省略分号成为可能，但也可能带来一些隐蔽问题。

实践建议：

- 要么统一写分号；
- 要么统一不写分号，但使用 Prettier、ESLint 等工具保证风格稳定；
- 遇到 `return`、`throw`、`break`、`continue` 等语句时，不要随意换行；
- 不写分号时，特别注意以 `(`、`[`、`` ` ``、`+`、`-`、`/` 开头的语句。
